...
use std::thread; // to get thread library
...
fn main() {
    ...
    let x1 = thread::spawn(move || map_data(&val1));
    let x2 = thread::spawn(move || map_data(&val2));
    
    let intermed_1 = x1.join().unwrap();
    let intermed_2 = x2.join().unwrap();
    intermediate_sums.push(intermed_1);
    intermediate_sums.push(intermed_2);
  ...
  //let mut values : Vec<Vec<usize>> = Vec::new();
	//let mut y_vals : Vec<thread::JoinHandle<usize>> = Vec::new();
	//let mut intermed : Vec<usize> = Vec::new();

	intermediate_sums.clear();

  for i in 0..num_partitions {
		let val = ys[i].clone();
		let y_vals = thread::spawn(move || map_data(&val));
		let intermed = y_vals.join().unwrap();
		intermediate_sums.push(intermed);
  }
	
  println!("Intermediate sums = {:?}", intermediate_sums);

  // REDUCE STEP: Process the intermediate result to produce the final result
  let sum2 = reduce_data(&intermediate_sums);
  println!("Sum = {}", sum2);
}
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
  ...
  for j in 0..num_partitions {
		...
		if j < remainder {
			...
			//for i in i..(i + partition_len + 1){
      //  x.push(v[i]);
    	//}
		}
		else {
			loop {
				if i == (partition_len + next_num) {
					break
				}
				else {
					x.push(v[i]);
